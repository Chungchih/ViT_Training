...
def main(args):
    utils.init_distributed_mode(args)  # not using DM.
    ...
    if 'quant' in args.model:
        model = create_model(
            ...
            input_noise_std_w=args.input_noise_std_w/2,
...
if __name__ == '__main__':
    ...
    #args.resume = './resumed_ckpt/deit_base_patch16_224-b5f2ef4d.pth'
    #args.model = 'deit_base_patch16_224'
    args.drop_path = 0
    args.wbits = 4
    args.abits = 4
    args.headwise = True
    args.input_noise_std = 0.3
    args.input_noise_std_w = 0.3
    args.output_noise_std = 0.01
    ...
    #errors = [0,0.05,0.1,0.15,0.2,0.25,0.3]
    errors = [0,0.1,0.15,0.2,0.3]
    acc1,acc5,loss = [],[],[]
    for noise_e in errors:
        for noise_w in errors:
            args.input_noise_std = noise_e
            args.input_noise_std_w = noise_w
            test_stats = main(args)
            acc1.append(test_stats['acc1'])
            acc5.append(test_stats['acc5'])
            loss.append(test_stats['loss'])