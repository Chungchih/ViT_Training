...
class QuantMlp(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.,
                 ...
                 input_noise_std=0, input_noise_std_w=0, output_noise_std=0, phase_noise_std=0,
        ...
        self.fc1 = QuantLinear(in_features, hidden_features, nbits=wbits, nbits_a=abits, mode=Qmodes.layer_wise,
                               offset=offset, input_noise_std=input_noise_std, input_noise_std_w=input_noise_std_w, output_noise_std=output_noise_std, phase_noise_std=phase_noise_std,
...
class QuantAttention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0., proj_drop=0.,
                 wbits=32, abits=32, headwise=False, offset=False, 
                 input_noise_std=0, input_noise_std_w=0, output_noise_std=0, phase_noise_std=0,
        ...
        self.input_noise_std_w = input_noise_std_w
    ...
    def add_input_noise_embedding(self, x):
    ...
    def add_input_noise_weight(self, x):
        # the noise std is 2sigma not 1sigma, so should be devided by 2
        if self.input_noise_std_w > 1e-5:
            # add mul noise here
            noise = torch.randn_like(x).mul(
                (self.input_noise_std_w)).mul(x.data.abs())
            x = x + noise
        return x
    ...
    def forward(self, x):
        ...
        if not self.training and self.phase_noise_std > 1e-5:
            ...
            k = self.add_input_noise_embedding(k)  # add noise
            q = self.add_input_noise_weight(q)  # add noise
        ...
        if not self.training and self.phase_noise_std > 1e-5:
            ...
            v = self.add_input_noise_embedding(v)  # add noise
            attn = self.add_input_noise_weight(attn)  # add noise
...
class QuantBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 ...
                 wbits=-1, abits=-1, headwise=False, input_noise_std=0, input_noise_std_w=0, output_noise_std=0, phase_noise_std=0,
        ...
        self.attn = QuantAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop,
                                   ...
                                   input_noise_std=input_noise_std, input_noise_std_w=input_noise_std_w, output_noise_std=output_noise_std, phase_noise_std=phase_noise_std,
                                   enable_wdm_noise=enable_wdm_noise, num_wavelength=num_wavelength, channel_spacing=channel_spacing,
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList([
            QuantBlock(
                ...
                headwise=headwise, input_noise_std=input_noise_std, input_noise_std_w=input_noise_std_w, output_noise_std=output_noise_std, phase_noise_std=phase_noise_std,
